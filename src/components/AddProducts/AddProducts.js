...
function AddProducts({ addNewProduct, availableProducts }) {
    ...
    const getAndSendNewProduct = () => {
        const isAlreadyOnList = availableProducts.some(product => product.nazwa === nazwa && product.kategoria === kategoria && product.produktSpozywczy === produktSpozywczy)
    ...
    return (
        <div className={styles.Wrapper}>
            ...
            <button
                disabled={!nazwa || !kategoria}
                onClick={getAndSendNewProduct}> Dodaj </button>
        </div>
    );
}
